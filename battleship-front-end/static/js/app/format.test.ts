...
describe('String', () => {
  ...
  describe('format', () => {
    ...
    it('should not follow reference change - example2', () => {
      assert_.equals('/{2}-{1}-5/', '/{0}-{1}-{2}/'.format('{2}', '{1}', 5));
    });

    it('should not fail on missing reference - none found', () => {
      assert_.equals('/a-b-c/', '/a-b-c/'.format('x', 'y', 5));
    });

    it('should not fail on missing reference - some found', () => {
      assert_.equals('/y-b-c/', '/{1}-b-c/'.format('x', 'y', 5));
    });

    it('should fill - multiple references', () => {
      assert_.equals('/x-x-y/', '/{0}-{0}-{1}/'.format('x', 'y'));
    });
