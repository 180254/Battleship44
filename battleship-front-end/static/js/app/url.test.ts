...
describe('UrlEx', () => {
  ...
  describe('param', () => {
    const verifyParamMethod: (
      locHref: string,
      name: string,
      expValue: string | undefined
    ) => void = (locHref, name, expValue) => {
    ...
    it('should not extract param - as not provided, but has misleading path', () => {
      verifyParamMethod('http://www.example.com/c=d/?a=b', 'c', undefined);
    });
  });
  ...
  describe('url', () => {
    const verifyUrlMethod: (
      locPath: string,
      params: UrlParam[],
      expected: string
    ) => void = (locPath, params, expected) => {
      if ((locPath.match(/\//g) || []).length < 3) {
        throw new Error(
          "incorrect locPath. use '{0}' instead of '{1}'".format(
